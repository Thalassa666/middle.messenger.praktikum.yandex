...
export function loginValidation(event: Event) {
    validateInput(
        event,
        /^[a-zA-Z0-9_-]{3,20}$/,
        "Логин должен содержать от 3 до 20 символов, только латинские буквы, цифры, дефис и/или подчеркивание.",
    );
}
...
export function phoneValidation(event: Event) {
    validateInput(
        event,
        /^\+?\d{10,15}$/,
        "Телефон должен содержать от 10 до 15 цифр, возможно начиная с плюса.",
    );
}
